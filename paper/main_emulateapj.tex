...
\tablecaption{Map fidelity vs.\ mean cloud cover (fiducial campaign;
mean over three seeds).\label{tab:clouds}}
\tablewidth{0pt}
\tablehead{\colhead{Method} & \colhead{$f_c{=}0$} & \colhead{0.22} &
\colhead{0.38} & \colhead{0.56} & \colhead{0.72}}
\startdata
\multicolumn{6}{c}{Pearson $r$}\\[2pt]
TDI (this work)        & 0.990 & 0.572 & 0.432 & 0.334 & 0.263 \\
B3 white-noise GLS     & 0.991 & 0.543 & 0.402 & 0.308 & 0.239 \\
B2 phase-binned coadd  & 0.669 & 0.328 & 0.215 & 0.129 & 0.052 \\[4pt]
\multicolumn{6}{c}{SSIM}\\[2pt]
...
\enddata
\tablecomments{Realized mean covers quoted; nominal $f_c=0$, 0.25,
0.40, 0.55, 0.70. All methods receive identical data, the same
climatological de-biasing, and constants frozen on a held-out
calibration seed.}
\end{deluxetable}
...
correlation rises monotonically from $r=0.15$ (4 long dwells) to
$r=0.58$ (64 short revisits)---a factor $\sim$4, at {\it identical}
photon cost. Coronal noise per visit grows as $t_s^{-1/2}$ but remains
...
\tablecaption{The five nearest confirmed potentially habitable planets
(and one solar-type alternative) as SGL imaging targets. Observing
geometry evaluated at $z=650$~AU, $d=1$~m, $\lambda=1\,\mu$m.
\label{tab:targets}}
\tablewidth{0pt}
\tablehead{
\colhead{Planet} & \colhead{Host (SpT)} & \colhead{$z_0$} &
\colhead{$M\sin i$} & \colhead{$R_p$\tablenotemark{a}} &
\colhead{$S$} & \colhead{$P$} &
\colhead{$D_{\rm img}$} & \colhead{$\mathrm{SNR_C}$} &
\colhead{Focal direction\tablenotemark{b}} & \colhead{$\beta_{\rm ecl}$} &
\colhead{$v_{\rm img}$} & \colhead{$\Delta v_{90}$\tablenotemark{c}} \\
 & & \colhead{(pc)} & \colhead{($M_\oplus$)} & \colhead{($R_\oplus$)} &
\colhead{($S_\oplus$)} & \colhead{(d)} & \colhead{(km)} &
\colhead{(1800 s)} & \colhead{(J2000)} & \colhead{(deg)} &
\colhead{(m s$^{-1}$)} & \colhead{(km s$^{-1}$)}}
\startdata
Proxima Cen b & Proxima (M5.5V) & 1.30 & 1.07 & 1.02 & 0.65 & 11.19 &
31.5 & 659 & $02^{\rm h}30^{\rm m}$, $+62^\circ41'$ & $+44.8$ & 114 & 5.8 \\
Ross 128 b & Ross 128 (M4V) & 3.38 & 1.35 & 1.09 & 1.38 & 9.87 &
12.9 & 576 & $23^{\rm h}48^{\rm m}$, $-00^\circ48'$ & $+0.5$ & 51 & 2.9 \\
GJ 1061 d & GJ 1061 (M5.5V) & 3.67 & 1.64 & 1.15 & 0.69 & 13.03 &
12.6 & 280 & $15^{\rm h}36^{\rm m}$, $+44^\circ31'$ & $+60.8$ & 39 & 1.7 \\
Teegarden c & Teegarden's Star (M7V) & 3.83 & 1.11 & 1.03 & 0.37 & 11.41 &
10.8 & 129 & $14^{\rm h}53^{\rm m}$, $-16^\circ53'$ & $-0.3$ & 35 & 1.7 \\
GJ 273 b & Luyten's Star (M3.5V) & 3.80 & 2.89 & 1.35 & 1.06 & 18.65 &
14.2 & 486 & $19^{\rm h}27^{\rm m}$, $-05^\circ14'$ & $+16.5$ & 44 & 1.3 \\
\hline
$\tau$ Cet e\tablenotemark{d} & $\tau$ Ceti (G8.5V) & 3.60 & 3.93 & 1.47 &
1.71 & 162.9 & 16.3 & 901 & $13^{\rm h}44^{\rm m}$, $+15^\circ56'$ &
$+24.8$ & 31 & 0.11 \\
\enddata
\tablenotetext{a}{Estimated from $M\sin i$ via $R\propto M^{0.28}$
\citep{chenkipping17}; none of these planets transit.}
\tablenotetext{b}{Antipode of the host's J2000 position (quoted to
arcminute precision); the usable focal segment extends along this
direction from $z\simeq548$~AU outward, with 650--900~AU the practical
operating range \citep{tur26prop}. $\beta_{\rm ecl}$ is the ecliptic
latitude of the focal direction.}
\tablenotetext{c}{Propellant cost of continuously tracking the
planetary image's orbital sweep for 90 days,
$\Delta v_{90}=a_{\rm img}\times90$~d; sampling strategies that let the
image sweep past a picket of spacecraft \citep{tt22dyn} reduce this
dramatically.}
\tablenotetext{d}{Radial-velocity candidate, not confirmed; included as
the nearest potentially habitable planet of a {\it solar-type} star.
Planet parameters: \citet{anglada16,faria22} (Proxima b);
\citet{bonfils18} (Ross 128 b); \citet{dreizler20} (GJ 1061 d);
\citet{zechmeister19,dreizler24} (Teegarden c);
\citet{astudillo17} (GJ 273 b); \citet{feng17} ($\tau$ Cet e).
Instellations from the discovery papers.}
\end{deluxetable*}
...
\end{equation}
where $p_0=9.08~\mu$N\,m$^{-2}$ and $\sigma_{\rm tot}$ is the
